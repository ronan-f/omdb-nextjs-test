...
const MetaData = ({ username, postedAt }) => {
    ...
    return (
        <Box className={classes.metadata} bgcolor="primary.main">
            <Avatar alt="Steve Jobs" src="https://www.glassdoor.com/blog/app/uploads/sites/2/jobs.jpg" />
...
const MovieInfo = ({ movie, rating }) => {
    ...
    return <CardContent className={classes.info}>

        <Typography className={classes.title} color="textPrimary" gutterBottom>
            {movie.title}
        </Typography>
        <Rating name="read-only" value={rating} readOnly />
        <img height="180px" width="120px" src={movie.poster} alt="Movie poster" />
    </CardContent>
}

const ReviewCard = ({ review, movie, rating, username, postedAt }) => {
    const classes = useStyles();
